...
bias_pred_direct <- rowMeans((predictors.overview$simulated.predictors - predictors.overview$direct_estimated)^2)
bias_pred_two_step <- rowMeans((predictors.overview$simulated.predictors - predictors.overview$two_step_estimated)^2)
variance_pred <- apply(predictors.overview$simulated.predictors, 1, var)
MSE_pred_direct <- variance_pred + bias_pred_direct
MSE_pred_two_step <- variance_pred + bias_pred_two_step


plot(x = MSE_pred_direct, y = MSE_pred_two_step, 
     xlim = c(0, max(c(MSE_pred_direct, MSE_pred_two_step))), 
...
bias_sens_two_step <- rowMeans((annotator.overview$true_anno_sens - annotator.overview$two_step_mean_sens)^2)
variance_sens <- apply(annotator.overview$true_anno_sens, 1, var)
MSE_sens_direct <- variance_sens + bias_sens_direct
MSE_sens_two_step <- variance_sens + bias_sens_two_step
...
bias_spec_direct <- rowMeans((annotator.overview$true_anno_spec - annotator.overview$direct_mean_spec)^2)
bias_spec_two_step <- rowMeans((annotator.overview$true_anno_spec - annotator.overview$two_step_mean_spec)^2)
variance_spec <- apply(annotator.overview$true_anno_spec, 1, var)
MSE_spec_direct <- variance_spec + bias_spec_direct
MSE_spec_two_step <- variance_spec + bias_spec_two_step
